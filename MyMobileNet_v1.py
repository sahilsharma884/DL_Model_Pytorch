...
class MyMobileNet_v1(nn.Module):
    def __init__(self, in_fts=3, num_filter=32, width_multiplier=1, resolution_multiplier=1, num_classes=1000):
        ...
        for l in self.nlayer_filter:
            if type(l) == list:
                if len(l) == 2:  # (num_times, out_channel)
                    for _ in range(l[0]):
                        block[str(index)] = Depthwise_Separable_Conv(self.in_fts, int(l[1]))
                        index += 1
                else:  # stride(2,2)
                    block[str(index)] = Depthwise_Separable_Conv(self.in_fts, int(l[0]), stride=(2, 2))
                    self.in_fts = int(l[0])
                    index += 1
            else:
                block[str(index)] = Depthwise_Separable_Conv(self.in_fts, int(l))
                self.in_fts = int(l)
                index += 1
...
if __name__ == '__main__':
    resolution_multiplier = 0.714
    H = 224
    x = torch.randn((2, 3, 224, 224))
    model = MyMobileNet_v1(width_multiplier=1)
    summary(model, (1, 3, 192, 160))
